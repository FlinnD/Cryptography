...
def writeOutput():
	with open('output.txt', 'w') as output:
		if(outputx[-1] == float('inf') and outputy[-1] == float('inf')):
			output.write('P\' is: ({},{}). You need to re-select n'.format(outputx[-1],outputy[-1]))
			
		else:
			output.write('P\' is: ({},{})'.format(outputx[-1],outputy[-1]))
		'''
		i = 1
